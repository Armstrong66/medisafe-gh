...
COPY pyproject.toml setup.py requirements.txt requirements-local.txt constraints.txt README.md ./
...
RUN python -m pip install --upgrade pip \
    ...
    && if [ "$INSTALL_LOCAL" = "true" ]; then python -m pip install -r requirements-local.txt -c constraints.txt; fi \
